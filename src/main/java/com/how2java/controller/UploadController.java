...
//            return videoId;
    ...
    /**
     * @param data         分片文件
     * @param fileType     文件类型 video/avi audio/mp3
     * @param name         文件名称(newName),由Client生成,即NewName 如
     *                     MICRO_CLASS_1502179979829.mp4
     * @param total        分片总数
     * @param index        当前分片数
     * @param  文件关联的MicroClass主键,通过先保存基本信息取得并返回(DataFile中的OwerID)
     * @param oldName      由用户自己输入的节目名称，需要与文件对应起来，如 实现两个100年奋斗目标
     * @param seq          文件的顺序,如第一集对应1，第二集对应2....
...
//    uploadSlice
    ...
    public Integer uploadSlice(@RequestBody MultipartFile data, String fileType, String name, Integer total, Integer index,
                               String oldName, int seq, HttpServletRequest request) throws Exception {
...
//                    dataFileService.saveDataFile(dataFile);
                ...
                if (index.equals(total)) {// 说明已经成功上传一个文件
